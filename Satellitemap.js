...
console.log('Loaded satellitemap.js');
...
function fetchAndShowSatelliteImage() {
    ...
    .then(data => {
        if (data.status === "OK") {
            ...
            const mapUrl = `https://maps.googleapis.com/maps/api/staticmap?center=${lat},${lng}&zoom=16&size=600x600&scale=4&maptype=satellite&key=${apiKey}`;
